<div class="container my-4">
  <div class="card p-3 shadow-lg mb-3" style="background-color:#2e7d32; color:white; border-radius:1rem;">

    <!-- Header torneo -->
    <h4 class="text-center fw-bold mb-1">World Othello Championship 2025</h4>
    <p class="text-center mb-3">Ankara (TR) – 13 novembre 2025</p>

    <div class="row g-4">

      <!-- Nero -->
      <div class="col-12 col-md-6">
        <div class="p-3 bg-dark bg-opacity-25 rounded">
          <div class="d-flex justify-content-between">
              <h5 class="fw-bold">⚫ Signorini Claudio <img src="https://flagcdn.com/24x18/it.png" alt="Italia"></h5>
              <h5 class="fw-bold">38</h5>
          </div>
        </div>
      </div>

      <!-- Bianco -->
      <div class="col-12 col-md-6">
        <div class="p-3 bg-dark bg-opacity-25 rounded">
          <div class="d-flex justify-content-between">
              <h5 class="fw-bold">⚪ Qvist Jessen Claus <img src="https://flagcdn.com/24x18/dk.png" alt="Danimarca"></h5>
             <h5 class="fw-bold">26</h5>
        </div>
      </div>

    </div>

    <div>
    <img src="Turno06.jpg" alt="Foto della partita" class="img-fluid rounded" />
    <div class="text-muted small">Foto: <i>Chu Chee Chin</i></div>
    </div>

    <!-- Note opzionali -->
    <div class="mt-4">
      <div style="background-color: white; color: black; border-radius: 10px; padding: 12px; margin: 8px 0;"></div>
    </div>

  </div>
</div>

<h2>La partita commentata</h2>

<board data-type="show" data-label="commento" data-file="01-pihlajapuro-signorini.json"
       data-caption="Partita commentata."></board>

<h2>Tocca a te</h2>
